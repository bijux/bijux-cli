    ...
    match contract_id {
        "STATUS-CONTRACT-GENERATE-MAINTAINER-CONTROL-PLANE-REPORTS" => {
            ...
            let command_samples = fs::read_to_string(
                workspace_root.join("artifacts/status/maintainer_control_plane_samples.json"),
            )
            .ok()
