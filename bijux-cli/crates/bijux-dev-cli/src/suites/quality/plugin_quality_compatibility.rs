...
pub(super) fn run(workspace_root: &Path, contract_id: &str) -> Option<Value> {
    match contract_id {
        "STATUS-CONTRACT-GENERATE-COMPATIBILITY-SHIM-REPORTS" => {
            ...
            let alias_literals = registry
                .split("let aliases = BTreeMap::from([")
                .nth(1)
                .and_then(|s| s.split("]);").next())
                .map(parse_quoted)
                .unwrap_or_default();
            let mut alias_pairs = alias_literals
                .chunks(2)
                .filter_map(|pair| {
            ...
            let aliases = alias_pairs
                                        ...
                                        .map(|(alias, canonical)| {
                                            if alias.starts_with("dev ") {
                                                json!({"alias":alias,"canonical":canonical,"classification":"temporary","justification":"legacy maintainer shortcut remains for compatibility contract","removal_condition":"remove when canonical bijux-dev-cli path has stable parity coverage","evidence_links":["artifacts/status/command_migration_matrix.json","artifacts/parity/command_parity_matrix.json"]})
