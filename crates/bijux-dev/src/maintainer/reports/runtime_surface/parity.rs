...
fn ensure_command_matrix(workspace_root: &Path) -> Value {
    ...
    let payload = json!({
        ...
        "generator": "crates/bijux-dev/src/maintainer/parity.rs::ensure_command_matrix",
        "commands": commands,
        "groups": groups,
        "plugin_lifecycle": {
...
fn ensure_parity_diffs(workspace_root: &Path, matrix: &Value) -> Value {
    ...
    let payload = json!({
        "generated_at": stable_generated_at(),
        "generator": "crates/bijux-dev/src/maintainer/parity.rs::ensure_parity_diffs",
        "diffs": rows,
    });
    write_json_if_changed(&path, &payload);
    payload
...
fn ensure_law_reports(workspace_root: &Path, matrix: &Value, diffs: &Value) {
    ...
    for name in [
        ...
        let payload = json!({
            "generated_at": stable_generated_at(),
            "generator": "crates/bijux-dev/src/maintainer/parity.rs::ensure_law_reports",
