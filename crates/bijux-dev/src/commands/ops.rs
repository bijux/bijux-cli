...
pub(super) fn run_repo_runtime_scope_reports(
    ...
    let core_api_report = format!(
        r#"# Core Public API Surface Report
...
"#
    ...
    let runtime_api_report = format!(
        r#"# Runtime Public API Surface Report
...
pub(super) fn run_runtime_module_triage_guard() -> Result<(), String> {
    ...
    let required = [
        ...
        "docs/reports/foundation/CORE_PUBLIC_API_SURFACE.md",
        "docs/reports/foundation/RUNTIME_PUBLIC_API_SURFACE.md",
...
pub(super) fn run_foundation_review_report() -> Result<(), String> {
    ...
    let report = json!({
        ...
        "has_foundation_governance_posture": root.join("docs/reports/foundation/FOUNDATION_GOVERNANCE_POSTURE.md").exists(),
    });
    println!("{}", serde_json::to_string_pretty(&report).map_err(|err| err.to_string())?);
    Ok(())
}
