...
fn temp_dir(name: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!("bijux-history-matrix-{name}-{}", std::process::id()));
...
fn history_root_listing_no_file_one_record_many_records_and_ordering() {
    ...
    fs::write(&one, serde_json::to_string(&vec![entry("status", 1.0)]).expect("json")).expect("write one");
    ...
    let many_entries = vec![entry("version", 1.0), entry("status", 2.0), entry("plugins list", 3.0)];
...
fn history_text_json_yaml_quiet_and_no_color_modes() {
    ...
    fs::write(&path, serde_json::to_string(&vec![entry("status", 1.0)]).expect("json")).expect("write");
    ...
    let yaml = run_with_env(&["history", "--format", "yaml", "--pretty"], &[("BIJUXCLI_HISTORY_FILE", h)]);
...
fn history_limit_path_override_and_repeated_run_determinism() {
    ...
    fs::write(
        &path,
        serde_json::to_string(&vec![entry("one", 1.0), entry("two", 2.0), entry("three", 3.0)]).expect("json"),
    )
    .expect("write");
...
fn history_help_and_exit_discipline_for_root_and_clear() {
    ...
    assert!(String::from_utf8(root_help.stdout).expect("utf-8").contains("Usage: bijux-rs history"));
    assert!(root_help.stderr.is_empty());

    let clear_help = run(&["history", "clear", "--help"]);
    assert_eq!(clear_help.status.code(), Some(0));
    assert!(String::from_utf8(clear_help.stdout).expect("utf-8").contains("Usage: bijux-rs history clear"));
