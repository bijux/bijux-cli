...
fn tmp_dir(label: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!("bijux-cross-state-{label}-{}", std::process::id()));
...
fn config_mutations_are_visible_across_binary_bridge_and_repl_reads() {
    ...
    let read_bridge = bridge_json(&["cli", "config", "get", "shared_key", "--config-path", &config_text]);
...
fn plugins_history_memory_and_paths_views_are_consistent_across_binary_and_bridge() {
    ...
    let runtime_bin = json_stdout(&run(&["dev", "cli", "runtime-identity", "--format", "json", "--no-pretty"]));
    let runtime_bridge =
...
fn state_path_overrides_propagate_consistently_for_config_path_views() {
    ...
    let bin = json_stdout(&run(&["--config-path", &config_text, "cli", "paths", "--format", "json", "--no-pretty"]));
    let bridge = bridge_json(&["--config-path", &config_text, "cli", "paths", "--format", "json", "--no-pretty"]);
