...
fn temp_dir(name: &str) -> PathBuf {
    let base = std::env::temp_dir().join(format!("bijux-config-hardening-{}-{}", name, std::process::id()));
...
fn config_truncation_duplicate_keys_line_endings_whitespace_and_null_byte_fail_cleanly() {
    ...
    let exits = [a.status.code(), b.status.code(), c.status.code(), d.status.code(), e.status.code(), f.status.code()];
...
fn config_set_clear_unset_failures_preserve_previous_content_as_rollback_proof() {
    ...
    let clear_fail = run(&["cli", "config", "clear", "--config-path", config_path.to_str().expect("utf-8")]);
    let unset_fail = run(&[
        "cli",
        "config",
        "unset",
...
fn config_clear_and_unset_retry_are_idempotent_after_transient_write_failure() {
    ...
    let first_clear = run(&["cli", "config", "clear", "--config-path", config_path.to_str().expect("utf-8")]);
    fs::set_permissions(&dir, fs::Permissions::from_mode(0o755)).expect("chmod 755");
    ...
    let second_clear = run_ok_json(&["cli", "config", "clear", "--config-path", config_path.to_str().expect("utf-8")]);
...
fn invalid_utf8_config_file_is_reported_cleanly() {
    ...
    let out = run(&["cli", "config", "reload", "--config-path", config_path.to_str().expect("utf-8")]);
