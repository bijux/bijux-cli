...
fn temp_dir(name: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!("bijux-config-deep-{name}-{}", std::process::id()));
...
fn config_key_normalization_and_parse_behavior_are_stable_across_repeated_inputs() {
    ...
    let first = run_json(&["cli", "config", "list", "--config-path", &path, "--format", "json", "--no-pretty"]);
    let second = run_json(&["cli", "config", "list", "--config-path", &path, "--format", "json", "--no-pretty"]);
...
fn config_export_and_load_preserve_semantic_content_and_roundtrip_exact_values() {
    ...
    assert_eq!(
        run(&["cli", "config", "set", "BIJUXCLI_ALPHA=hello world", "--config-path", &source_path]).status.code(),
        Some(0)
    );
    assert_eq!(
        run(&["cli", "config", "set", "BIJUXCLI_QUOTED='quoted value'", "--config-path", &source_path]).status.code(),
        Some(0)
    );

    let json_export =
        run_json(&["cli", "config", "list", "--config-path", &source_path, "--format", "json", "--no-pretty"]);
    let yaml_out = run(&["cli", "config", "list", "--config-path", &source_path, "--format", "yaml", "--pretty"]);
    let text_out = run(&["cli", "config", "list", "--config-path", &source_path, "--format", "text"]);
    assert_eq!(yaml_out.status.code(), Some(0));
    assert_eq!(text_out.status.code(), Some(0));

    assert_eq!(
        run(&["cli", "config", "export", &exported_path, "--config-path", &source_path]).status.code(),
        Some(0)
    );
    assert_eq!(
        run(&["cli", "config", "load", &exported_path, "--config-path", &target_path]).status.code(),
        Some(0)
    );
    let loaded_json =
        run_json(&["cli", "config", "list", "--config-path", &target_path, "--format", "json", "--no-pretty"]);
    assert_eq!(json_export, loaded_json, "load+export should preserve semantic content");

    let roundtrip_get = run_json(&["cli", "config", "get", "alpha", "--config-path", &source_path, "--format", "json", "--no-pretty"]);
...
fn config_unset_clear_and_repeated_mutations_follow_expected_semantics() {
    ...
    let missing = run(&["cli", "config", "get", "alpha", "--config-path", &path, "--format", "json", "--no-pretty"]);
    assert_eq!(missing.status.code(), Some(2));

    assert_eq!(run(&["cli", "config", "clear", "--config-path", &path]).status.code(), Some(0));
    let listed = run_json(&["cli", "config", "list", "--config-path", &path, "--format", "json", "--no-pretty"]);
...
fn root_and_cli_config_path_override_behavior_is_identical_for_list() {
    ...
    let root_list = run(&["config", "list", "--config-path", &path, "--format", "json", "--no-pretty"]);
    let cli_list = run(&["cli", "config", "list", "--config-path", &path, "--format", "json", "--no-pretty"]);
...
#[test]
fn config_doctor_and_state_doctor_agree_on_corrupted_config_findings() {
    let root = temp_dir("doctor-agreement");
    let config = root.join("config.env");
    let path = config.to_string_lossy().to_string();
    fs::write(&config, "BIJUXCLI_OK=1\nBAD_LINE\n").expect("write malformed config");

    let doctor = run_json(&["dev", "cli", "doctor", "--config-path", &path, "--format", "json", "--no-pretty"]);
    let state_doctor =
        run_json(&["dev", "cli", "state-doctor", "--config-path", &path, "--format", "json", "--no-pretty"]);

    let doctor_config_issues = doctor["issues"]["config"].as_array().expect("doctor config issues");
    let state_config_issues = state_doctor["doctor"]["issues"].as_array().expect("state doctor issues");
