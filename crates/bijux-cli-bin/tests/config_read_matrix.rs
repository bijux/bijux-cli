...
fn temp_dir(name: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!("bijux-config-read-matrix-{name}-{}", std::process::id()));
...
fn root_config_list_empty_one_multiple_duplicate_comments_and_malformed_behavior() {
    ...
    let empty_out = run(&["config", "--format", "json", "--no-pretty", "--config-path", empty.to_str().expect("utf-8")]);
    ...
    let one_out = run(&["config", "--format", "json", "--no-pretty", "--config-path", one.to_str().expect("utf-8")]);
    ...
    let multi_out = run(&["config", "--format", "json", "--no-pretty", "--config-path", multi.to_str().expect("utf-8")]);
    ...
    let dup_out = run(&["config", "--format", "json", "--no-pretty", "--config-path", duplicate.to_str().expect("utf-8")]);
    ...
    let malformed_out = run(&["config", "--format", "json", "--no-pretty", "--config-path", malformed.to_str().expect("utf-8")]);
...
fn config_get_existing_missing_invalid_with_path_and_env_override() {
    ...
    let found = run(&["cli", "config", "get", "alpha", "--format", "json", "--no-pretty", "--config-path", file.to_str().expect("utf-8")]);
    ...
    let missing = run(&["cli", "config", "get", "missing", "--format", "json", "--no-pretty", "--config-path", file.to_str().expect("utf-8")]);
    assert_eq!(missing.status.code(), Some(2));
    assert!(missing.stdout.is_empty());
    assert!(!missing.stderr.is_empty());

    let invalid = run(&["cli", "config", "get", "bad-key", "--format", "json", "--no-pretty", "--config-path", file.to_str().expect("utf-8")]);
    assert_eq!(invalid.status.code(), Some(2));
    assert!(invalid.stdout.is_empty());
    assert!(!invalid.stderr.is_empty());

    let path_override = run_with_env(
        &["cli", "config", "get", "alpha", "--format", "json", "--no-pretty", "--config-path", file.to_str().expect("utf-8")],
...
fn config_get_json_yaml_text_quiet_and_no_color_behavior() {
    ...
    let json_out = run(&["cli", "config", "get", "alpha", "--format", "json", "--no-pretty", "--config-path", path]);
    assert_eq!(json_out.status.code(), Some(0));
    let _: Value = serde_json::from_slice(&json_out.stdout).expect("json");

    let yaml_out = run(&["cli", "config", "get", "alpha", "--format", "yaml", "--pretty", "--config-path", path]);
    ...
    let text_out = run(&["cli", "config", "get", "alpha", "--format", "text", "--config-path", path]);
