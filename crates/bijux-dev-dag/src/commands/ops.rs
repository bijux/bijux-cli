...
pub(super) fn markdown_contains_path(rel_path: &str, markdown_files: &[PathBuf], root: &Path) -> bool {
...
pub(super) fn run_repo_artifact_capability_reports(matrix_out: &Path, model_out: &Path) -> Result<(), String> {
...
pub(super) fn run_evidence_domain_verify(domain: &str, required_paths: &[&str]) -> Result<(), String> {
    let root = repo_root()?;
    run_evidence_metadata_validate()?;
...
pub(super) fn run_evidence_consumer_reports(assets_out: &Path, consumers_out: &Path) -> Result<(), String> {
    let root = repo_root()?;
    let assets = load_registry_assets(&root)?;
...
pub(super) fn collect_contract_files(dir: &Path, out: &mut Vec<PathBuf>) -> Result<(), String> {
    if !dir.exists() {
        return Ok(());
    }
    for entry in fs::read_dir(dir).map_err(|err| err.to_string())? {
...
#[derive(Debug, Deserialize)]
pub(super) struct ErrorCodeRegistry {
    version: u64,
...
pub(super) struct ErrorCodeEntry {
    code: String,
    category: String,
    owner: String,
    description: String,
...
pub(super) fn load_error_code_registry(root: &Path) -> Result<ErrorCodeRegistry, String> {
