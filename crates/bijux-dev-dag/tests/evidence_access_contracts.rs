...
fn registry_access_uses_approved_helpers_only() {
    ...
    let allowlist = BTreeSet::from([
        "crates/bijux-dev-dag/src/commands/evidence_access.rs".to_string(),
        "crates/bijux-dev-dag/src/commands/evidence_registry.rs".to_string(),
...
fn no_legacy_scenario_roots_are_referenced_by_runtime_sources() {
    ...
    collect_files(&root.join("crates"), &mut files);
    let allowlist = BTreeSet::from([
        "crates/bijux-dev-dag/src/commands/mod.rs".to_string(),
        "crates/bijux-dev-dag/tests/evidence_consumer_integrity_contracts.rs".to_string(),
        "crates/bijux-dev-dag/tests/evidence_access_contracts.rs".to_string(),
    ]);
