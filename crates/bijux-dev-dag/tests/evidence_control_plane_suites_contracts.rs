...
fn verify_commands_include_replay_and_release_set() {
    ...
    for token in [
        ...
        assert!(
            ...
            "missing evidence control-plane token: {token}"
        );
    }
}
...
fn evidence_suite_policy_covers_required_verify_surface() {
    ...
    let ids: BTreeSet<String> = suites
        .iter()
        .map(|entry| entry["id"].as_str().expect("suite id").to_string())
        .collect();
    for required in [
        "evidence-schema",
        "evidence-registry",
        ...
        assert!(
            ...
            "missing evidence suite policy id: {required}"
        );
    }
}
...
fn release_evidence_set_assets_exist_and_are_registered() {
    ...
    for field in ["blocking_assets", "advisory_assets"] {
        for entry in release_set[field].as_array().expect("release set array") {
            ...
            assert!(
                root.join(id).exists(),
                "release evidence asset path is missing: {id}"
            );
            assert!(
                ids.contains(id),
                "release evidence asset id is not present in registry: {id}"
            );
        }
    }
