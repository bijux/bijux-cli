...
fn schema_example_dir(schema_rel: &str) -> PathBuf {
    let stem = Path::new(schema_rel)
        ...
        .expect("schema stem");
    root()
        .join("evidence/operator/examples/stable_json")
        .join(stem)
}
...
fn assert_schema_example_lockstep(schema_rel: &str) {
    ...
    let minimal: Value =
        serde_json::from_str(&fs::read_to_string(&minimal_path).expect("read minimal"))
            .expect("parse minimal");
    let maximal: Value =
        serde_json::from_str(&fs::read_to_string(&maximal_path).expect("read maximal"))
            .expect("parse maximal");

    assert_eq!(
        minimal["schema"], schema_rel,
        "minimal schema link mismatch"
    );
    assert_eq!(
        maximal["schema"], schema_rel,
        "maximal schema link mismatch"
    );
    ...
    for key in required {
        assert!(
            min_data.contains_key(&key),
            "minimal example missing required field {key} in {schema_rel}"
        );
        assert!(
            max_data.contains_key(&key),
            "maximal example missing required field {key} in {schema_rel}"
        );
    }
}
...
fn generated_json_output_reports_and_registries_exist() {
    for rel in [
        ...
        assert!(
            ...
            "missing generated json governance artifact: {rel}"
        );
    }
}
...
fn schema_example_and_lockstep_gap_reports_are_zero() {
    let example_gap = fs::read_to_string(
        root().join("docs/reports/foundation/schema_without_example_output_report.md"),
    )
    .expect("read schema gap report");
    assert!(example_gap.contains("Missing schema examples: 0"));

    let lockstep_gap = fs::read_to_string(
        root().join("docs/reports/foundation/commands_without_json_lockstep_report.md"),
    )
...
fn stable_command_registry_matches_policy_families() {
    ...
    let registry =
        fs::read_to_string(root().join("docs/reference/STABLE_JSON_OUTPUT_COMMAND_REGISTRY.md"))
            .expect("read command registry");
    for family in families {
        assert!(
            registry.contains(&format!("`{family}`")),
            "registry missing family {family}"
        );
    }
}
