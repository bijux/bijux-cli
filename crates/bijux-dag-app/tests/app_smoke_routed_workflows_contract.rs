...
fn smoke_validate_plan_run_inspect_replay_diff() {
    ...
    run_ok(&["dag".to_string(), "plan".to_string(), "explain".to_string(), graph_path.display().to_string()]);
    run_ok(&[
        "dag".to_string(),
        "run".to_string(),
...
fn smoke_artifact_hash_inspect_trace() {
    ...
    run_ok(&["dag".to_string(), "hash".to_string(), "artifact".to_string(), artifact_path.display().to_string()]);
    run_ok(&[
        "dag".to_string(),
