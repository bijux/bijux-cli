...
pub(super) fn run_repo_runtime_scope_reports(
    ...
    let dev_tests = root.join("crates/bijux-core-dev/tests");
...
pub(super) fn run_evidence_consumers_verify() -> Result<(), String> {
    ...
    collect_all_files(&root, &mut files)?;
    let ignore_paths = [
        "crates/bijux-core-dev/src/commands/mod.rs",
...
pub(super) fn run_planner_alignment_guard() -> Result<(), String> {
    ...
    let required = [
        ...
        "crates/bijux-core-dev/tests/planner_hardening_contracts.rs",
    ...
    let commands = fs::read_to_string(root.join("crates/bijux-core-dev/src/commands/mod.rs"))
        .map_err(|err| err.to_string())?;
...
pub(super) fn run_scheduler_invariants_guard() -> Result<(), String> {
    ...
    let required = [
        ...
        "crates/bijux-core-dev/tests/scheduler_hardening_contracts.rs",
    ];
    let mut missing = Vec::new();
    for rel in required {
        if !root.join(rel).exists() {
...
pub(super) fn run_state_machine_contract_guard() -> Result<(), String> {
    ...
    let required = [
        ...
        "crates/bijux-core-dev/tests/state_machine_hardening_contracts.rs",
    ];
    let mut missing = Vec::new();
    for rel in required {
        if !root.join(rel).exists() {
...
pub(super) fn run_backend_contract_guard() -> Result<(), String> {
    ...
    let required = [
        ...
        "crates/bijux-core-dev/tests/backend_hardening_contracts.rs",
    ];
    let mut missing = Vec::new();
    for rel in required {
        if !root.join(rel).exists() {
...
pub(super) fn run_artifact_hardening_guard() -> Result<(), String> {
    ...
    for rel in [
        ...
        "crates/bijux-core-dev/tests/run_dir_import_export_hardening_contracts.rs",
...
pub(super) fn run_cache_evolution_guard() -> Result<(), String> {
    ...
    let required = [
        ...
        "crates/bijux-core-dev/tests/cache_hardening_contracts.rs",
    ];
    let mut missing = Vec::new();
    for rel in required {
        if !root.join(rel).exists() {
...
pub(super) fn run_replay_contract_guard() -> Result<(), String> {
    ...
    let required = [
        ...
        "crates/bijux-core-dev/tests/replay_hardening_contracts.rs",
    ];
    let mut missing = Vec::new();
    for rel in required {
        if !root.join(rel).exists() {
...
pub(super) fn run_sacred_execution_flow_guard() -> Result<(), String> {
    ...
    let required = [
        ...
        "crates/bijux-core-dev/tests/sacred_execution_hardening_contracts.rs",
    ];
    let mut missing = Vec::new();
    for rel in required {
        if !root.join(rel).exists() {
...
pub(super) fn run_foundation_verification_guard() -> Result<(), String> {
    ...
    for rel in [
        ...
        "crates/bijux-core-dev/src/commands/mod.rs",
        "crates/bijux-core-dev/src/suites/repo.rs",
    ] {
        if !root.join(rel).exists() {
