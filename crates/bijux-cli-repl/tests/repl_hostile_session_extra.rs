...
use bijux_cli_repl::{
    ...
    startup_repl, startup_repl_with_diagnostics, ReplEvent, ReplInput, ReplStream,
};
...
fn temp_path(name: &str, ext: &str) -> PathBuf {
    std::env::temp_dir().join(format!("bijux-repl-hostile-extra-{name}-{}.{}", std::process::id(), ext))
}
...
fn startup_with_corrupted_history_registry_missing_paths_and_large_history_is_resilient() {
    ...
    let (_session_with_diag, _startup, diagnostics) = startup_repl_with_diagnostics("default", None, &["community"]);
    assert_eq!(diagnostics.len(), 1);

    let huge = temp_path("huge-history", "json");
    let huge_lines: Vec<String> = (0..50_000).map(|i| format!("status {i}")).collect();
    fs::write(&huge, serde_json::to_string(&huge_lines).expect("serialize huge")).expect("write huge history");
    ...
    let first = execute_repl_line(&mut huge_session, "status").expect("first command").expect("frame");
...
fn ctrl_c_eof_mode_switch_and_no_color_behavior_are_stable_in_one_session() {
    ...
    execute_repl_input(&mut session, ReplInput::Line(":set format json".to_string())).expect("json mode");
    let json_frame = execute_repl_line(&mut session, "status").expect("json status").expect("frame");
    assert!(json_frame.content.trim_start().starts_with('{'));

    execute_repl_input(&mut session, ReplInput::Line(":set format yaml".to_string())).expect("yaml mode");
    let yaml_frame = execute_repl_line(&mut session, "status").expect("yaml status").expect("frame");
    assert!(yaml_frame.content.contains("status:"));

    execute_repl_input(&mut session, ReplInput::Line(":set format text".to_string())).expect("text mode");
    let text_frame = execute_repl_line(&mut session, "status").expect("text status").expect("frame");
    assert!(text_frame.content.contains("\"status\""));

    let no_color = execute_repl_line(&mut session, "help status --color never").expect("help").expect("frame");
    ...
    let interrupted_plugin = execute_repl_input(&mut session, ReplInput::Interrupt).expect("interrupt plugin");
    assert!(matches!(interrupted_plugin, ReplEvent::Interrupted(_)));

    execute_repl_input(&mut session, ReplInput::Line("status \\".to_string())).expect("multiline open");
    let eof = execute_repl_input(&mut session, ReplInput::Eof).expect("eof");
    assert!(matches!(eof, ReplEvent::Exit(_)));
}

#[test]
fn plugin_management_state_doctor_and_broken_completion_source_do_not_crash() {
    let (mut session, _) = startup_repl("default", None);
    ...
    let state_doctor = execute_repl_line(
        &mut session,
        &format!("dev cli state-doctor --format json --no-pretty --config-path {}", config.display()),
    )
    .expect("state doctor command")
    .expect("state doctor frame");
    assert!(state_doctor.content.contains("doctor"));

    let (_broken, _startup, diagnostics) = startup_repl_with_diagnostics("default", None, &["community", "missing"]);
    assert!(!diagnostics.is_empty());

    let suggestions = completion_candidates(&session, "sta");
    assert!(suggestions.iter().any(|c| c == "status"));

    let recovered = execute_repl_line(&mut session, "status").expect("recovery").expect("recovery frame");
    assert_eq!(recovered.stream, ReplStream::Stdout);

    let _ = fs::remove_file(config);
}

fn load_history_like_user(session: &mut bijux_cli_repl::ReplSession) {
    // Mirrors user startup behavior with tolerant history loading.
    let _ = bijux_cli_repl::load_history(session);
}
