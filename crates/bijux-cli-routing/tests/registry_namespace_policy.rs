...
fn official_reserved_namespaces_take_precedence() {
    ...
    for ns in ["cli", "dev", "help", "version", "doctor", "repl", "plugins", "completion", "inspect"] {
        let result = registry.register_plugin_namespace(ns);
        assert!(matches!(result, Err(RouteError::Reserved(_))), "expected reserved rejection for {ns}");
    }
}
...
fn user_plugin_namespace_rejection_rules_apply() {
    let mut registry = RouteRegistry::default();
    assert!(registry.register_plugin_namespace("status").is_err(), "builtin root collision must fail");
    ...
    assert!(registry.register_plugin_namespace("community").is_err(), "duplicate plugin namespace must fail");
}

#[test]
fn plugin_name_collision_with_builtin_command_root_is_rejected() {
    let mut registry = RouteRegistry::default();
    let err = registry.register_plugin_namespace("config").expect_err("config root must be protected");
