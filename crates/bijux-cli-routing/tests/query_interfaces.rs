...
fn route_and_registry_queries_are_stable_across_repeated_runs() {
    ...
    let first_routes = bijux_cli_routing::inventory::route_inventory(&registry);
    let second_routes = bijux_cli_routing::inventory::route_inventory(&registry);
    assert_eq!(first_routes, second_routes);

    let first_registry = bijux_cli_routing::inventory::registry_inventory(&registry);
    let second_registry = bijux_cli_routing::inventory::registry_inventory(&registry);
