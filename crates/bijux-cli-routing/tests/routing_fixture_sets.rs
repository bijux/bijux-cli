...
fn all_cli_subcommand_fixtures_resolve() {
    ...
    for path in read_lines("tests/fixtures/cli_subcommands.txt") {
...
fn all_dev_cli_subcommand_fixtures_resolve() {
    let registry = RouteRegistry::default();
    for path in read_lines("tests/fixtures/dev_cli_subcommands.txt") {
...
fn plugin_namespace_fixture_commands_resolve_after_registration() {
    ...
    for path in read_lines("tests/fixtures/plugin_namespace_commands.txt") {
...
fn invalid_command_suggestions_match_fixtures() {
    let text = fs::read_to_string("tests/fixtures/invalid_command_suggestions.json")
        .expect("fixture should exist");
