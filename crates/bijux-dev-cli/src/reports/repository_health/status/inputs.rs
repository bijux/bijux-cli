...
fn ensure_status_report(status_report: Value, parity_rows: &[Value]) -> Value {
    ...
    if let Some(obj) = report.as_object_mut() {
        obj.entry("commands".to_string())
            .or_insert_with(|| Value::Array(commands.clone()));
...
fn synthesize_migration_matrix(status_report: &Value, parity_rows: &[Value]) -> Value {
    ...
    let rust_complete = commands
        .iter()
        .filter(|row| row["status"] == "rust-complete")
        .count();
    let rust_partial = commands
        .iter()
        .filter(|row| row["status"] == "rust-partial")
        .count();
    let python_only = commands
        .iter()
        .filter(|row| row["status"] == "python-only")
        .count();
    let intentional = commands
        .iter()
        .filter(|row| row["status"] == "intentionally-different")
        .count();
...
fn ensure_migration_sidecars(command_migration: &mut Map<String, Value>, matrix: &Value) {
    if command_migration
        .get("bridge_duplicate_law_report")
    ...
    let rows = matrix
        ...
        .cloned()
        .unwrap_or_default();
    let rust_partial: Vec<Value> = rows
        .iter()
        .filter(|row| row["status"] == "rust-partial")
        .cloned()
        .collect();
    let python_only: Vec<Value> = rows
        .iter()
        .filter(|row| row["status"] == "python-only")
        .cloned()
        .collect();
    let intentional: Vec<Value> = rows
        .iter()
        .filter(|row| row["status"] == "intentionally-different")
        .cloned()
        .collect();
