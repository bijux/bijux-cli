...
use bijux_dev_cli::{
    cockpit, config, contracts, control_plane, crate_health, docs_audit, env, maintenance_audit,
    package_health, parity, python, registry, repo, route_audit, routes, runtime_identity,
...
fn all_command_groups_build_expected_top_level_keys() {
    ...
    assert!(
        status::build_report(&root, maintenance_audit::build_inventory_report(&root))
            .get("status_report")
            .is_some()
    );
    assert!(
    ...
    assert!(maintenance_audit::build_report(inventory)
        .get("maintenance")
        .is_some());
