...
fn legacy_status_maps_to_cli_status_and_resolves() {
    ...
    let target = registry
        .resolve(&intent.normalized_path)
...
fn removed_dev_aliases_no_longer_normalize_to_dev_cli_commands() {
    let cases = [
        (
            vec!["dev".to_string(), "docs".to_string()],
            vec!["dev", "docs"],
        ),
        (
            vec!["dev".to_string(), "env".to_string()],
            vec!["dev", "env"],
        ),
        (
            vec!["dev".to_string(), "contracts".to_string()],
            vec!["dev", "contracts"],
        ),
        (
...
fn removed_dev_alias_paths_resolve_as_unknown_and_canonical_path_still_resolves() {
    ...
    let legacy_err = registry
        .resolve(&legacy)
        .expect_err("legacy alias should be unknown");
    assert!(matches!(
        legacy_err,
        bijux_cli::routing::registry::RouteError::Unknown(_)
    ));
...
fn removed_dev_aliases_for_atlas_di_and_list_products_are_unknown() {
    ...
    for path in removed {
        let err = registry
            .resolve(&path)
            .expect_err("removed alias should resolve as unknown");
        assert!(matches!(
            err,
            bijux_cli::routing::registry::RouteError::Unknown(_)
        ));
    ...
    let canonical_paths = [
        ...
        vec![
            "dev".to_string(),
            "cli".to_string(),
            "crate-health".to_string(),
        ],
        vec![
            "dev".to_string(),
            "cli".to_string(),
            "docs-prune-plan".to_string(),
        ],
        vec![
            "dev".to_string(),
            "cli".to_string(),
            "maintenance-audit".to_string(),
        ],
    ];
