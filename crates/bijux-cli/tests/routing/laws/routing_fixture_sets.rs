...
#[test]
fn all_dev_cli_subcommand_fixtures_are_delegated_outside_runtime_registry() {
    let registry = RouteRegistry::default();
    for path in read_lines("tests/data/fixtures/routing/dev_cli_subcommands.txt") {
        let resolved = registry.resolve(&path);
        assert!(resolved.is_err(), "dev cli fixture should be delegated outside runtime registry");
    }
}
