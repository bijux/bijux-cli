...
#[test]
fn cli_doctor_legacy_conflicts_flag_tracks_detected_conflicts() {
    ...
    let payload: serde_json::Value = serde_json::from_str(&stdout).expect("valid json");
    assert_eq!(payload["install"]["legacy_installer_conflicts"], true);
}
...
fn runtime_identity_reports_ambiguous_active_binary_selection() {
    ...
    write_executable(&first.join(&executable));
    write_executable(&second.join(&executable));
