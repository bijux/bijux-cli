...
fn temp_dir(label: &str) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "bijux-diagnostics-trust-{label}-{}-{nanos}",
        std::process::id()
    ));
...
fn dev_cli_registry_env_parity_crate_health_and_docs_audit_reflect_live_truth() {
    ...
    let registry_snapshot: Value = serde_json::from_str(include_str!(
        "../../../data/golden/ported/dev_cli_registry.json"
    ))
    ...
    assert!(parity["binary_bridge"]["cases"]
        .as_array()
        .is_some_and(|v| !v.is_empty()));

    let crate_health = parse_json(
        &run(&[
            "dev",
            "cli",
            "crate-health",
...
fn doctor_plugin_doctor_and_runtime_identity_provide_actionable_diagnostics_for_problem_cases() {
    ...
    let text_report = plugin_health["machine_report"]["text_report"]
        .as_str()
        .unwrap_or_default();
    ...
    if runtime["active_binary_selection_is_ambiguous"]
        .as_bool()
        .unwrap_or(false)
    {
...
fn diagnostics_text_is_boring_and_json_is_machine_friendly() {
    ...
    for promotional in [
        ...
        assert!(
            ...
            "diagnostics text should remain neutral"
        );
    }
    ...
    for cmd in diagnostics_commands {
        ...
        assert_eq!(
            out.status.code(),
            Some(0),
            "json diagnostics failed for {cmd:?}"
        );
        assert!(
            out.stderr.is_empty(),
            "json diagnostics should keep stderr empty for {cmd:?}"
        );
        let payload = parse_json(&out.stdout);
        assert!(
            payload.is_object(),
            "diagnostics payload should be object for {cmd:?}"
        );
