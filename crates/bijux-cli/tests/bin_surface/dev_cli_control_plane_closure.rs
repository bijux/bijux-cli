...
fn text_snapshot_suite_keeps_head_lines_stable_for_primary_dashboards() {
    ...
    assert_eq!(
        snapshot + "\n",
        include_str!("../snapshots/dev_cli_control_plane_text_heads.json"),
...
fn script_replacement_suite_tracks_migrated_workflows() {
    ...
    for path in required {
        let found = scripts
            .iter()
            .any(|row| row.get("path") == Some(&Value::String(path.to_string())));
        assert!(
            found,
            "missing script inventory row for replacement candidate: {path}"
        );
    }
}
