...
fn run_bin(args: &[&str]) -> io::Result<Output> {
    ...
    for _ in 0..3 {
        match Command::new(env!("CARGO_BIN_EXE_bijux-rs"))
            .args(args)
            .output()
        {
...
fn concurrent_config_readers_and_writers_preserve_file_shape_and_recoverability() {
    ...
    assert!(body
        .lines()
        .all(|line| line.starts_with("BIJUXCLI_") && line.contains('=')));
...
fn concurrent_history_plugin_registry_and_memory_reads_remain_stable() {
    ...
    let plugins_dir = PathBuf::from(
        envs.iter()
            .find(|(k, _)| *k == "BIJUXCLI_PLUGINS_DIR")
            .expect("plugins env")
            .1
            .clone(),
    );
    let history = PathBuf::from(
        envs.iter()
            .find(|(k, _)| *k == "BIJUXCLI_HISTORY_FILE")
            .expect("history env")
            .1
            .clone(),
    );
