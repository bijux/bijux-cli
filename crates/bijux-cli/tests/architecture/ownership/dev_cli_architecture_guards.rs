...
fn core_dev_cli_routes_delegate_to_dev_cli_module_helpers() {
    ...
    assert!(
        dispatch_source.contains("let forwarded = if tool_namespace ==  ")
            && dispatch_source.contains("&argv[3..]"),
        "runtime dispatch must normalize canonical dev-cli delegation arguments"
    );
    assert!(
        dispatch_source.contains("else { &argv[2..] }"),
        "runtime dispatch must normalize delegated `dev <subcommand>` arguments"
    );
    assert!(
        dispatch_source.contains("return Some(delegate_dev_cli(forwarded));"),
        "runtime dispatch must delegate non-product dev routes to external dev-cli binary"
    );
    assert!(
