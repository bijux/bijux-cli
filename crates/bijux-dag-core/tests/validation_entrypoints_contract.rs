...
use bijux_dag_core::{
    parse_graph_strict,
    validate::{validate_graph, validate_schema, validate_semantics, validate_topology, validation_rule_registry},
};
...
fn validate_module_entrypoints_are_covered_and_consistent() {
    ...
    assert!(full.is_empty(), "expected no validation diagnostics for valid graph");
    ...
    assert!(!rules.is_empty(), "validation rule registry must not be empty");
