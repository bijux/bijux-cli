use crate::canonical::{
    error, is_valid_canonical_name, is_valid_output_path, severity_rank, warn,
};
...
impl Graph {
    pub fn validate_with_warnings(&self) -> Vec<ValidationDiagnostic> {
        ...
        for node in &self.nodes {
            ...
            if node.kind == crate::NodeKind::Container && !node.effects.contains(&Effect::Filesystem) {
                diagnostics.push(error(
                    "E1009",
            ...
            if node.retry.max_attempts > 0
                && (node.effects.contains(&Effect::Clock) || node.effects.contains(&Effect::Network))
            {
                ...
                if !has_seed && !self.nondeterminism_allowed {
                    diagnostics.push(error(
                        ...
                        Some(
                            "Provide inputs.random_seed or set nondeterminism_allowed=true".to_string(),
                        ),
                    ));
                }
            }
...
pub fn validate_schema(graph: &Graph) -> Vec<ValidationDiagnostic> {
    validate_graph(graph)
        .into_iter()
        .filter(|diag| matches!(classify_rule_domain(diag.code.as_str()), Some(ValidationDomain::Schema)))
        .collect()
}
...
pub fn validate_semantics(graph: &Graph) -> Vec<ValidationDiagnostic> {
    validate_graph(graph)
        .into_iter()
        .filter(|diag| matches!(classify_rule_domain(diag.code.as_str()), Some(ValidationDomain::Semantic)))
        .collect()
}
...
pub fn validate_topology(graph: &Graph) -> Vec<ValidationDiagnostic> {
    validate_graph(graph)
        .into_iter()
        .filter(|diag| matches!(classify_rule_domain(diag.code.as_str()), Some(ValidationDomain::Topology)))
        .collect()
}
    ...
    match value {
        ParamValue::Ref(spec) => {
            ...
            if let Some(node_output) = &spec.node_output {
                let target = graph.nodes.iter().find(|candidate| candidate.id == node_output.node_id);
                if target.is_none()
                    || !target
                        ...
                        .any(|output| output.name == node_output.path)
                {
                    diagnostics.push(error(
                        "E1021",
                        format!("unknown node output ref: {}.{}", node_output.node_id, node_output.path),
                        path.to_string(),
                        None,
                    ));
                }
                    ...
                    (order.get(node_output.node_id.as_str()), order.get(node.id.as_str()))
                {
                    if source_index >= current_index {
                        diagnostics.push(error(
                            "E1022",
                            format!("forward node output ref: {}.{}", node_output.node_id, node_output.path),
                            path.to_string(),
                            None,
                        ));
                    }
        ...
        ParamValue::Array(items) => {
            for (index, value) in items.iter().enumerate() {
                validate_param_value(value, graph, node, order, diagnostics, &format!("{path}/{index}"));
            }
        }
        ParamValue::Object(map) => {
            for (key, value) in map {
                validate_param_value(value, graph, node, order, diagnostics, &format!("{path}/{key}"));
            }
        }
