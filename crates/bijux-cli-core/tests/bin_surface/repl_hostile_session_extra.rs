...
fn plugin_management_state_doctor_and_broken_completion_source_do_not_crash() {
    ...
    let state_doctor = execute_repl_line(
        &mut session,
        &format!(
            "dev cli state-doctor --format json --no-pretty --config-path {}",
            config.display()
        ),
    )
...
fn load_history_like_user(session: &mut bijux_cli_core::repl::ReplSession) {
    // Mirrors user startup behavior with tolerant history loading.
    let _ = bijux_cli_core::repl::load_history(session);
}
