...
use bijux_cli_core::repl::{estimated_session_memory_bytes, startup_repl};
...
fn repl_startup_latency_with_plugin_completion_hooks_stays_within_budget() {
    let started = Instant::now();
    for _ in 0..10 {
        let (_session, _startup, diagnostics) = bijux_cli_core::repl::startup_repl_with_diagnostics(
            "benchmark",
            None,
