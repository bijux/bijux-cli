...
## Internal boundaries
- `src/runtime_core/`: orchestration boundary (`engine`, `scheduler`, `state`, planner bridge, execution context, invariants).
...
- `src/simulated_platform.rs`: explicit quarantine facade for modeled platform/distributed/product surfaces that are retained for evidence and contract coverage but are not part of the stable runtime root.
