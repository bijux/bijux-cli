...
- `make test-all`: runs full Rust suite including ignored tests.
- `make test-release`: runs `make test-all` plus release-critical evidence verification wrappers (`battle`, `cache`, `replay`, `compat`, `fault`, `perf`, `consumers`, `release-set`).
