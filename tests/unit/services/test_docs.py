...
def test_init_env_root(
    ...
    with patch.dict(os.environ, {"BIJUXCLI_DOCS_DIR": str(env_root)}):
        d = Docs(mock_observability, mock_serializer, mock_telemetry)
...
def test_init_none_root(
    ...
    with patch.object(Path, "mkdir"):
        d = Docs(mock_observability, mock_serializer, mock_telemetry, root=None)
        assert d._root.name == "docs"
...
def test_render_uses_serializer(docs: Docs, mock_serializer: SerializerProtocol) -> None:
    ...
    cast(Mock, mock_serializer).dumps.assert_called_once_with(
...
def test_init_root_type(
    ...
    with patch("bijux_cli.services.diagnostics.docs.Path", return_value=actual_root):
        d = Docs(mock_observability, mock_serializer, mock_telemetry, root=root)
...
def test_init_mkdir_exists(
    ...
    Docs(mock_observability, mock_serializer, mock_telemetry, root=temp_root)
...
def test_init_mkdir_fail(
    ...
    with (
        ...
        Docs(mock_observability, mock_serializer, mock_telemetry, root="/fail")
