...
and simulated batch backends. The repository now includes a modeled SLURM lane
that executes through shared runtime adapters while keeping scheduler semantics
explicitly simulated rather than cluster-backed.
...
execution-mode reporting, and the modeled SLURM execution lane exercised by:
...
semantics, execution-mode classification, modeled SLURM request fields, or
restart-recovery claims must update this model and the linked runtime tests in
the same change.
