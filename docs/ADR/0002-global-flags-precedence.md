...
2. **Quiet** (`-q` / `--quiet`)
   ...
   * Overrides **debug**, **format**, and **pretty/no-pretty** for output suppression, but not for exit codes.
...
3. **Debug** (`--log-level debug`)
   ...
   * Implicitly enables runtime metadata output.
