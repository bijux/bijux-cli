...
In current `v0.4.0` releases, declared output paths are checked before
execution starts. Paths that are not normalized relative paths, paths that try
to escape through `../x`, and paths that traverse a symlinked existing parent
component are rejected before a shell subprocess or container receives a
writable target.
