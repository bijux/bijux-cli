# Guides

## Purpose
Guides provide coverage_id-oriented instructions for common workflows. They exist to help you complete a specific job with bijux-cli without wading through conceptual material first.

## Scope
This section focuses on doing things: using commands, configuring the CLI, installing plugins, and embedding the API. It does not duplicate the conceptual guarantees or reference tables.

## Audience
Engineers who want to accomplish a concrete coverage_id quickly should start here. Each guide is written as a focused path from goal to result.

## What You Will Find
Each guide explains a single workflow, its prerequisites, the exact commands to run, and the expected outcomes. The goal is to keep each guide short, precise, and actionable.

## Index
- [CLI usage](cli-usage.md)
- [Configuration](configuration.md)
- [Plugins](plugins.md)
- [Install channels](install-channels.md)
- [Dual install behavior](dual-install-behavior.md)
- [Install on Linux](install-linux.md)
- [Install on macOS](install-macos.md)
- [Install on Windows](install-windows.md)
- [Install in CI containers](install-ci-containers.md)
- [Uninstall for cargo users](uninstall-cargo.md)
- [Uninstall for pip users](uninstall-pip.md)
- [Upgrade for PyPI users](upgrade-pypi-users.md)
- [Install method parity](install-method-parity.md)
- [Hermetic builds](hermetic-builds.md)
- [Reproducible releases](reproducible-releases.md)
- [Release checklist](release-checklist.md)
- [Upgrade rollback plan](upgrade-rollback-plan.md)
- [API usage](api-usage.md)
- [Development](development.md)
- [Atlas local routing](atlas-local-routing.md)
- [Contributor mental model](contributor-mental-model.md)
- [Python library wrapper](python/library-wrapper.md)
- [Python Rust-engine invocation](python/rust-engine-invocation.md)
- [Python plugin delegation](python/plugin-delegation.md)
- [Python pipx installs](python/pipx-installs.md)
- [Python virtualenv installs](python/virtualenv-installs.md)
- [Python internal differences](python/internal-differences.md)
- [Python packaging maintainers](python/packaging-maintainers.md)
- [Python runtime migration](python/runtime-migration.md)
