...
|        3 | `--log-level debug`            | Full diagnostics; implies `--log-level debug` and forces `--pretty`. |
|        4 | `-f`, `--format` `<json\|yaml>` | Structured output; invalid value → exit code 2.              |
|        5 | `--pretty` / `--no-pretty`      | Indentation control (default: `--pretty`).                   |
|        6 | `-v`, `--log-level debug`               | Include runtime metadata; implied by `--log-level debug`.     |
...
* Need more detail? Use `--log-level debug` or `--log-level debug` (adds pretty printing and diagnostics).
