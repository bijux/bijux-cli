...
SNAPSHOT_DIR = ROOT / "crates" / "bijux-cli-bin" / "tests" / "snapshots"
...
def find_doctor_snapshots() -> list[str]:
    ...
    return [f"crates/bijux-cli-bin/tests/snapshots/{name}" for name in candidates if (SNAPSHOT_DIR / name).exists()]
