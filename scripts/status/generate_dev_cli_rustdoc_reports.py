...
def run(command: list[str], fmt: str) -> str:
    proc = subprocess.run(
        ["cargo", "run", "-q", "-p", "bijux-cli", "--", *command, "--format", fmt, "--no-pretty" if fmt == "json" else ""],
        cwd=ROOT,
        capture_output=True,
        text=True,
        check=False,
...
def run_json(command: list[str]) -> dict:
    proc = subprocess.run(
        ["cargo", "run", "-q", "-p", "bijux-cli", "--", *command, "--format", "json", "--no-pretty"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        check=True,
...
def main() -> int:
    ...
    audit_text = subprocess.run(
        ["cargo", "run", "-q", "-p", "bijux-cli", "--", "dev", "cli", "rustdoc", "audit", "--format", "text"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        check=True,
