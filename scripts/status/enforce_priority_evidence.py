#!/usr/bin/env python3
"""Enforce evidence-only priority planning requirements."""
...
def main() -> int:
    ...
    required = [
        STATUS / "priority_plan.json",
        STATUS / "priority_plan.txt",
        STATUS / "simplification_priorities.json",
        STATUS / "simplification_priorities.txt",
    ...
    if missing:
        failures.append(f"missing priority-plan artifacts: {', '.join(missing)}")

    priority_plan = read_json(STATUS / "priority_plan.json")
    policy = priority_plan.get("evidence_first_policy", {}) if isinstance(priority_plan, dict) else {}
    minimalism_priorities = read_json(STATUS / "simplification_priorities.json")
    minimalism_policy = (
        minimalism_priorities.get("evidence_first_policy", {})
        if isinstance(minimalism_priorities, dict)
        else {}
    )

    if policy.get("manual_curated_priority_lists_allowed") is not False:
        failures.append("manual curated priority-plan priorities must be disabled")
    ...
    if minimalism_policy.get("roadmap_requires_generated_artifacts") is not True:
    ...
    for path in ROOT.rglob("*priority*plan*"):
        if not path.is_file():
            continue
        ...
        if rel.startswith("artifacts/status/"):
            continue
        if rel.endswith("priority_plan.json") or rel.endswith("priority_plan.txt"):
            continue
        manual_priority_files.append(rel)
    if manual_priority_files:
        failures.append(
            "manual priority-plan files detected outside generated artifacts: "
            + ", ".join(sorted(manual_priority_files)[:20])
        )

    for msg in failures:
        print(f"PRIORITY PLAN GATE FAILURE: {msg}")
