...
TEST_FILES = [
    ROOT / "crates" / "bijux-cli-bin" / "tests" / "diagnostics_command_matrix.rs",
    ROOT / "crates" / "bijux-cli-bin" / "tests" / "diagnostics_contract_consistency.rs",
    ROOT / "crates" / "bijux-cli-bin" / "tests" / "diagnostics_deep_behavior_extra.rs",
]
...
def main() -> None:
    ...
    expected_contracts = json.loads(
        (ROOT / "crates" / "bijux-cli-bin" / "tests" / "snapshots" / "ported" / "dev_cli_contracts.json").read_text(
            encoding="utf-8"
        )
    )
    expected_routes = json.loads(
        (ROOT / "crates" / "bijux-cli-bin" / "tests" / "snapshots" / "ported" / "dev_cli_routes.json").read_text(
            encoding="utf-8"
        )
    )
