...
def main() -> int:
    ...
    if contract:
        ...
        if missing_coverage_ids != 0:
            failures.append(f"stream discipline coverage_id coverage incomplete: missing_coverage_ids={missing_coverage_ids}")
    ...
    if drift:
        ...
        if drift.get("missing_coverage_ids"):
            failures.append(f"stream drift artifact has missing coverage_id coverage: {drift.get('missing_coverage_ids')}")
